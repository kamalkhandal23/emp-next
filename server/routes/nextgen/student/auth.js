...
// @desc    Student Login (DEPRECATED - use login.js instead)
// @route   POST /api/nextgen/student/login
// @access  Public
// NOTE: This route is disabled to avoid conflict with login.js
/* router.post('/login', [
  ...
  async (req, res) => {
    try {
      ...
      if (!student) {
        ...
        const studentId = `NGE${new Date().getFullYear()}${String(
          studentCount + 1
        ).padStart(3, '0')}`;
