...
export default function GradeExams() {
    ...
    const fetchSubmissions = async (examId) => {
        ...
        try {
            const token = localStorage.getItem("authToken");
            const res = await fetch(`http://localhost:5002/api/nextgen/exams/${examId}/submissions`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                },
    ...
    const handleGradeSubmission = async (submissionId, grade, feedback) => {
        ...
        try {
            const token = localStorage.getItem("authToken");
            const res = await fetch(`http://localhost:5002/api/nextgen/exams/grade/${submissionId}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
