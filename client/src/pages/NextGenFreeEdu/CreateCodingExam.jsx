...
function CreateCodingExam() {
  ...
  const handleTypeSelect = (qNum, type) => {
    ...
    setQuestionData((prev) => ({
      ...prev,
      [qNum]: prev[qNum] || { type, question: "", options: [], answer: "", testCase: "" },
    }));
  };
  ...
  const handleFinalizeCodingExam = async () => {
    ...
    if (!allQuestionsFilled) {
      alert('⚠️ Please fill in all question details before finalizing the coding exam.');
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        alert(`✅ Coding exam "${examName}" created successfully!\n\nExam ID: ${response.data.id}`);
      ...
      setSubmitError(error.message || 'Failed to create coding exam. Please try again.');
      alert(`❌ Error: ${error.message || 'Failed to create coding exam. Please try again.'}`);
    } finally {
                ...
                <div className="p-6 bg-yellow-50 border-t-4 border-yellow-400 rounded-lg shadow-lg">
                  ...
                  {!questionTypes[selectedQuestion] ? (
                    <div className="flex gap-4">
                      <button
                        onClick={() => handleTypeSelect(selectedQuestion, 'Coding')}
              ...
              <div className="mt-8 text-center">
                ...
                {submitSuccess && (
                  <div className="mb-4 p-4 bg-green-100 border border-green-400 text-green-700 rounded-lg">
                    <p className="font-semibold">✅ Coding exam created successfully!</p>
                    <p>Redirecting...</p>
                  </div>
                )}
